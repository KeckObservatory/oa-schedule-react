    ...
    if (new Date(range[0]).getTime() < firstDay && range[1] !== null){
      ...
      .then(data => {
        ...
        fetch("https://vm-www3build:53872/observers", {
              method: 'post',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({'Schedule': newsched, 'Start': new Date(range[0]).getTime(), 'End': end.setDate(end.getDate() + 1) })
            })
              .then(response => response.json())
              .then(data => {
                setSchedule([...data])
  ...
  const getSchedule = useCallback(() => {
    ...
    .then(data => {
      ...
      fetch("https://vm-www3build:53872/observers", {
        method: 'post',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({'Schedule': data, 'Start': data[0].Date, 'End': data[data.length].Date })
      })
        .then(response => response.json())
        .then(data => {
          setSchedule([...data])
